...
export default {
  ...
  methods: {
    explosion: function(event) {
      // Prop defaults.
      const amountMax = this.amountMax || 100;
      const amountMin = this.amountMin || amountMax;
      const borderRadiusStart = this.borderRadiusStart || '0';
      const borderRadiusEnd = this.borderRadiusEnd || '1px';
      const colorStart = this.colorStart || 'random';
      const colorEnd = this.colorEnd || 'random';
      const config = this.config || {};
      const defaults = this.defaults || TRUE;
      const durationMax = this.durationMax || 500;
      const durationMin = this.durationMin || durationMax;
      const ease = this.ease || 'easeInBounce';
      const easeBackgroundColor = this.easeBackgroundColor || this.ease;
      const easeBorderRadius = this.easeBorderRadius || this.ease;
      const easeFilter = this.easeFilter || this.ease;
      const easeHeight = this.easeHeight || this.ease;
      const easeWidth = this.easeWidth || this.ease;
      const easeOpacity = this.easeOpacity || this.ease;
      const easeRotate = this.easeRotate || this.ease;
      const easeX = this.easeX || this.ease;
      const easeY = this.easeY || this.ease;
      const filterStart = this.filterStart || 'blur(1px)';
      const filterEnd = this.filterEnd || 'blur(0px)';
      const opacityStart = this.opacityStart || 1;
      const opacityEnd = this.opacityEnd || 0;
      const radiusMax = this.radiusMax || 50;
      const radiusMin = this.radiusMin || radiusMax;
      const rotateMax = this.rotateMax || 0;
      const rotateMin = this.rotateMin || rotateMax * -1;
      const sizeWidthStart = this.sizeWidthStart || '2px';
      const sizeHeightStart = this.sizeHeightStart || sizeWidthStart;
      const sizeWidthEnd = this.sizeWidthEnd || '1px';
      const sizeHeightEnd = this.sizeHeightEnd || sizeWidthEnd;
